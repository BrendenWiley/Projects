...
range = obj.distanceP_HI.values           #phidp
azimuth = obj.azimuthP_HI.values[sweep]   #phidp

phase = obj.DifferentialPhase_HI.values[sweep]   #phidp
...
colormap = colortables.get_colortable('NWSStormClearReflectivity')  #phidp

figure, axis = plt.subplots(1, 1, figsize=(10, 9), subplot_kw=dict(projection=ccrs.PlateCarree()))

image = axis.pcolormesh(longitude, latitude, phase, vmin=-80, vmax = 80, cmap=colormap) #phidp
...
plt.title(f'{station}: {obj.DifferentialPhase_HI.name}', loc = 'left') #phidp
...
print(dir(obj.DifferentialReflectivity_HI))
