...
range = obj.distanceV_HI.values           #rho
azimuth = obj.azimuthV_HI.values[sweep]   #rho

SW = obj.SpectrumWidth_HI.values[sweep]   #rho
...
colormap = colortables.get_colortable('Carbone42')  #rho

figure, axis = plt.subplots(1, 1, figsize=(10, 9), subplot_kw=dict(projection=ccrs.PlateCarree()))

image = axis.pcolormesh(longitude, latitude, SW, vmin=-1, vmax = 20, cmap=colormap) #rho
...
plt.title(f'{station}: {obj.SpectrumWidth_HI.name}', loc = 'left') #phidp
