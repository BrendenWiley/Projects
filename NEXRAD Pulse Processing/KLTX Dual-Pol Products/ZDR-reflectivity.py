...
sweep = 0
range = obj.distanceR_HI.values          #zdr
azimuth = obj.azimuthR_HI.values[sweep]  #zdr

re = obj.Reflectivity_HI.values[sweep]   #zdr
...
colormap = colortables.get_colortable('NWSStormClearReflectivity')  #zdr

figure, axis = plt.subplots(1, 1, figsize=(10, 9), subplot_kw=dict(projection=ccrs.PlateCarree()))

image = axis.pcolormesh(longitude, latitude, re, vmin=-30, vmax = 79, cmap=colormap) #zdr
...
plt.title(f'{station}: {obj.Reflectivity_HI.name}', loc = 'left') #zdr
