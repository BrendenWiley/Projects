...
param.A         = 200;    %// Scaling factor
param.g         = 9.81 ;    %// gravitational constant

param.xLim = [-50 50] ;     %// domain limits X
param.yLim = [-50 50] ;     %// domain limits Y
param.zLim = [-20 20] ;
%% // Pre-set modes
...
% Example usage within the main script or a callback
x = 0; % Example X coordinate
y = 0; % Example Y coordinate

param = getappdata(h.fig, 'param'); % Retrieve the parameters from the GUI application data

wave_height = get_wave_height(x, y, time, param);
disp(['Wave height at (', num2str(x), ',', num2str(y), ') at time ', num2str(time), ' is: ', num2str(wave_height)]);
...
[pitch, roll, yaw] = calculate_angles(x, y, time, param);
...
function height = get_wave_height(x, y, time, param)

    % Initialize wave coefficients
    [H0, W, Grid_Sign] = initialize_wave(param);
    
    % Calculate the wave height matrix at the given time
    Z = calc_wave(H0, W, time, Grid_Sign);
    
    % Interpolate to find the height at the specific (x, y) coordinates
    xGrid = linspace(param.xLim(1), param.xLim(2), param.meshsize);
    yGrid = linspace(param.yLim(1), param.yLim(2), param.meshsize);
    height = interp2(xGrid, yGrid, Z, x, y, 'linear');
end

function [pitch, roll, yaw] = calculate_angles(x, y, time, param)

    % Get wave coefficients
    H0 = getappdata(gcf, 'H0');
    W = getappdata(gcf, 'W');
    Grid_Sign = getappdata(gcf, 'Grid_Sign');

    % Calculate wave height data
    Z = calc_wave(H0, W, time, Grid_Sign);
    ...
    [~, idxX] = min(abs(xGrid - x));
    [~, idxY] = min(abs(yGrid - y));

    % Estimate partial derivatives using equation 
    dZdx = (Z(idxY, idxX+1) - Z(idxY, idxX-1)) / (xGrid(idxX+1) - xGrid(idxX-1));
    dZdy = (Z(idxY+1, idxX) - Z(idxY-1, idxX)) / (yGrid(idxY+1) - yGrid(idxY-1));

    % Calculate pitch roll and yaw in degrees
    pitch = atan(dZdx) * 180 / pi;  % Rotation around Y-axis
    roll = atan(dZdy) * 180 / pi;   % Rotation around X-axis
    yaw = atan2(dZdy, dZdx) * 180 / pi; % Rotation around Z-axis 

    % Display angles
    disp(['Pitch: ', num2str(pitch), ' degrees']);
    disp(['Roll: ', num2str(roll), ' degrees']);
    disp(['Yaw: ', num2str(yaw), ' degrees']);
